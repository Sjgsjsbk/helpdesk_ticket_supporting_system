    ...
    <div class="col-md-12">
        <div class="nav-align-top mb-4">
            <ul class="nav nav-pills mb-3" role="tablist">
                ...
                <li class="nav-item">
                    <button type="button" class="nav-link" ng-init="Btn=2" id="2" ng-click="GetTickets(2);StoreStatusValue(2)" role="tab" data-bs-toggle="tab" data-bs-target="#navs-pills-top-home" aria-controls="navs-pills-top-home" aria-selected="false">
                        Processing
                        <label class="badge rounded-pill badge-center h-px-20 w-px-20 bg-danger">{{ProcessingCount}}</label>
                    </button>
                </li>
                <li class="nav-item">
                    <button type="button" class="nav-link" ng-click="GetTickets(3);StoreStatusValue(3)" id="3" role="tab" data-bs-toggle="tab" data-bs-target="#navs-pills-top-home" aria-controls="navs-pills-top-home" aria-selected="false">
                        Query Raised
                        <label class="badge rounded-pill badge-center h-px-20 w-px-20 bg-danger">{{QueryReopensCount }}</label>
                    </button>
                </li>
                <li class="nav-item">
                    <button type="button" ng-click="GetTickets(4);StoreStatusValue(4)" id="4" class="nav-link" role="tab" data-bs-toggle="tab" data-bs-target="#navs-pills-top-home" aria-controls="navs-pills-top-home" aria-selected="false">
                        Complete
                                ...
                                <tr ng-repeat="Ticket in ticketsDetails | orderBy:['-PriorityId','-RaisedOn'] | filter:searchText">
                                    <td>
                                        <div style="display: flex; align-items: center;">
                                            <i class="bx bxs-envelope me-2"></i> <span>{{Ticket.Code}}</span><label ng-if="Ticket.Is_Read == 2" style="color: red; margin-left: 5px;"><sup><i class="bx bx-comment-detail"></i></sup></label>
                                        ...
                                        <!-- Flexbox container for aligning Agent Name and Icon -->
                                        <div ng-show="Ticket.AgentName" style="display: flex; align-items: center;">
                                    ...
                                    <td>
                                        ...
                                        <div ng-if="Ticket.StatusId == 2 || Ticket.StatusId == 3 " style="display: flex; align-items: center; margin-left: 10px;">
                                            <div ng-init="combinedRemark = Ticket.QueryRemark && Ticket.ResponseRemark || Ticket.QueryRemark || Ticket.ResponseRemark || 'Comment..'">
                                            ...
                                            <div style="display: flex; align-items: center; margin-left: 10px;"> <!-- Add margin-left to create space -->
                                                ...
                                                <span class="badge badge-center bg-primary" ng-show='Ticket.ResponseRemark'>
                                                    <i class='bx bx-reply bx-flip-horizontal' style='font-size: 20px; cursor: pointer;' title="Reply" ng-click='GetQueryRespons(Ticket)'></i>
                                                </span>
                                            </div>
                                        </div>
                                    </td>
                                    <td><span class="badge bg-{{getPriorityIdColor(Ticket.PriorityId)}} me-1">{{Ticket.PriorityId == 1 ? 'Low' : (Ticket.PriorityId == 2 ? 'Medium' : 'High')}}</span></td>
...
<script>
    ...
    app.controller('TicketTblCtrl', function($scope, $rootScope, $http, $timeout, $filter, $interval) {
        ...
        $scope.GetTabTicketCounts = function() {
            var url = APIRootPath + "api/items/getTabTicketCount/" + $rootScope.UserUUID + "/" +   $rootScope.UserTypeId  + "/" + CategoryId + "/" + $rootScope.LinkedToId + "/" + $rootScope.MappedCustromerId;
        ...
        $scope.GetTickets = function(TicketStatus) {
            var url = APIRootPath + "api/customer/get_CustomerUserTickets/" + $rootScope.UserUUID + "/" +TicketStatus + "/" + CategoryId + "/" + $rootScope.MappedCustromerId;
            $http.get(url, urlconfig).then(
                function(response) {
                    $rootScope.ticketsDetails = response.data.tickets;
                    $scope.Tickets = $rootScope.ticketsDetails;
                    $rootScope.ticketsDetails = $scope.Tickets.filter(Ticket => Ticket.StatusId == TicketStatus && Ticket.CustomerId == GlobalFrmrootScope.MappedCustromerId && Ticket.RaisedById == $rootScope.CustomerUserId && Ticket.TicketCategoryId == CategoryId);
                    $rootScope.notificationList = $scope.Tickets.filter(Ticket => Ticket.CustomerId == GlobalFrmrootScope.MappedCustromerId && Ticket.RaisedById == $rootScope.CustomerUserId);
                    $scope.TicketFillter();
                },
                function(response) {
                    if (response.data.status != 401) {
        ...
        $scope.TicketFillter = function() {
            if ($scope.FrmTbl.ProductId != undefined && $rootScope.StatusValue != undefined) {
                $rootScope.ticketsDetails = $scope.Tickets.filter(Ticket => Ticket.ProductId == $scope.FrmTbl.ProductId && Ticket.StatusId == $rootScope.StatusValue && Ticket.CustomerId == GlobalFrmrootScope.MappedCustromerId && Ticket.RaisedById == $rootScope.CustomerUserId && Ticket.TicketCategoryId == CategoryId);
            } else if ($scope.FrmTbl.ProductId != undefined) {
                $rootScope.ticketsDetails = $scope.Tickets.filter(Ticket => Ticket.ProductId == $scope.FrmTbl.ProductId && Ticket.CustomerId == GlobalFrmrootScope.MappedCustromerId && Ticket.RaisedById == $rootScope.CustomerUserId && Ticket.TicketCategoryId == CategoryId);
            } else if ($rootScope.StatusValue != undefined) {
                $rootScope.ticketsDetails = $scope.Tickets.filter(Ticket => Ticket.StatusId == $rootScope.StatusValue && Ticket.CustomerId == GlobalFrmrootScope.MappedCustromerId && Ticket.RaisedById == $rootScope.CustomerUserId && Ticket.TicketCategoryId == CategoryId);
            } else {
        ...
        $scope.filterNotifications = function(notification) {
            return notification.QueryRemark && notification.ResponseRemark != '' ||
                !notification.QueryRemark && notification.ResponseRemark &&
                new Date(notification.QueryOn) > new Date(Date.now() - 24 * 60 * 60 * 1000);
        };

        $scope.getCustomerProductsList = function() {
            var url = APIRootPath + "api/GetCustomerProduct/" + $rootScope.UserUUID + "/" + $rootScope.MappedCustromerId;
            $http.get(url, urlconfig).then(
                function(response) {
    ...
    app.controller('queryresponsectrl', function($scope, $rootScope, $http, $timeout, $filter, $interval) {
        ...
        $scope.SubmitQuery = function() {
            ...
            if (form.reportValidity()) {
                ...
                if (form.reportValidity() && !$scope.SaveClicked) {
                    ...
                    // <i class='bx bx-loader-circle bx-spin' style='color:#ffffff' ></i>
        ...
        $rootScope.GetQueryRespons = function(Ticket) {
