...
<div class="row">
    <div class="d-flex justify-content-between align-items-center">
        <h4 class="fw-bold">
            <span class="text-muted fw-light">Product Detail</span>
        </h4>
        <a class="fw-bold" href="Customer" style="padding-bottom: 10px;"><button type="button" class="btn btn-dark">Back</button></a>
    </div>
</div>
<div class="row" ng-controller="ProductTblCtrl" ng-model="FrmTbl">
    <div class="col-12" style="padding-bottom: 20px;">
        <div class="card">
            <h5 class="card-header d-flex justify-content-between align-items-center">Customer Detail</h5>
            <div class="card-body" style="padding-bottom:15px; justify-content: space-evenly;">
                <div class="row">
                    <div class="mb-3 col-md-3">
                        <label for="Name" class="form-label col-md-12">Name</label>
                        <input class="form-control" type="text" name="ProductName" id="Name" ng-model="FrmDetail[0].CompanyName ? FrmDetail[0].CustomerFullName : FrmDetail.CustomerFullName" readonly />
                    </div>
                    <div class="mb-3 col-md-3">
                        <label for="Company" class="form-label col-md-12">Company</label>
                        <input class="form-control" type="text" name="CompanyName" id="Name" ng-model="FrmDetail[0].CompanyName ? FrmDetail[0].CompanyName : FrmDetail.CompanyName" readonly />
                    </div>
                    <div class="mb-3 col-md-4">
                        <label for="Company" class="form-label col-md-12">Email</label>
                        <input class="form-control" type="text" name="Email" id="Name" ng-model="FrmDetail[0].Email ? FrmDetail[0].Email : FrmDetail.Email" readonly />
                    </div>
                    <div class="mb-3 col-md-2">
                        <label for="Company" class="form-label col-md-12">Mobile</label>
                        <input class="form-control" type="text" name="CompanyName" id="Name" ng-model="FrmDetail[0].Mobile ? FrmDetail[0].Mobile : FrmDetail.Mobile" readonly />
                    </div>
                </div>
            </div>
        </div>
                ...
                <table class="table">
                    ...
                    <tbody class="table-border-bottom-0">
                        <tr ng-repeat="product in ProductDetails | filter:FrmTbl.searchText">
                            ...
                            <td><span style="cursor: pointer;" class="badge  bg-{{product.IsActive == 1 ? 'success':'danger'}}" ng-click="UpdateProductStatus(product)"><i class="bx bx-{{product.IsActive == 1 ? 'check':'x'}}"></i></span></td>
                            <td>
                                <a class="" href="javascript:void(0);" ng-click="EditProduct(product)" title="Edit"><i class="bx bx-edit-alt me-1"></i></a>
                                <a class="" ng-show="product.isexist == 0" href="javascript:void(0);" ng-click="DeleteProduct(product)" title="Delete"><i class="bx bx-trash me-1"></i></a>
                            </td>
                        </tr>
                    </tbody>
                </table>
...
<div class="row">
    <div class="mb-3 col-md-4">
        ...
        <input class="form-control" type="text" name="ProductName" id="Name" ng-model="FrmDetail[0].CompanyName ? FrmDetail[0].CompanyName : FrmDetail.CompanyName" readonly />
    </div>
    <div class="mb-3 col-md-3">
    ...
    <div class="mb-3 col-md-3">
        ...
        <select class="select2 form-select" ng-model="FrmNewProduct.CategoryId" id="CategoryId" required ng-change="clearErrorMessage()">
...
<script>
    ...
    app.controller('ProductTblCtrl', function($scope, $rootScope, $http, $timeout, $filter) {
        $scope.FrmProduct = {};
        $scope.FrmTbl = {};
        $rootScope.FrmDetail = JSON.parse(sessionStorage.getItem("customer")) || {};
        $rootScope.CustomerId = $scope.FrmDetail[0] && $scope.FrmDetail[0].Id ? $scope.FrmDetail[0].Id : $scope.FrmDetail.Id;
        ...
        $scope.GetProducts = function() {
            ...
            $http.get(url, urlconfig).then(
                function(response) {
                    ...
                    $scope.ProductDetails = $scope.ProductDetails.filter(Product => Product.CustomerId == $rootScope.CustomerId);
                },
                function(response) {
        ...
        $scope.AddProduct = function() {
            ...
            GlobalFrmProductScope.FrmNewProduct = [];
            $('#AddNewProductModal').modal('show');
    ...
    app.controller('FrmProductCtrl', function($scope, $rootScope, $http, $timeout, $filter) {
        ...
        // $scope.FrmNewProduct.CompanyName = $rootScope.FrmDetail[0].CompanyName;
        $rootScope.CustomerId = $scope.FrmDetail[0] && $scope.FrmDetail[0].Id ? $scope.FrmDetail[0].Id : $scope.FrmDetail.Id;
        // Other functions...
